...
class CNN:
    def __init__(self,alpha,batch_size,num_classes,num_features, num_channels):
        ...
        def __graph__():
            ...
            dense = tf.layers.dense(flat_1, units=8, activation = tf.nn.relu)
...
if __name__ == '__main__':
    ...
    num_features = 5000
    num_channels = 12
    model=CNN(alpha= 0.002, batch_size=64, num_classes= num_classes, num_features=num_features, num_channels=num_channels)
    model.train(checkpoint_path='C:/tmp/'+datetime.now().strftime('%Y%m%d_%H%M%S'),epochs=1000, log_path='C:/tmp/logs',train_data=train_data)
